...
class RuleSimulator(UserSimulator):
    ...
    def response_request(self, system_action):
        ...
        if len(system_action['request_slots'].keys()) > 0:
            ...
            elif slot in self.goal['request_slots'].keys() and slot not in self.state['rest_slots'] and slot in self.state[
                'history_slots'].keys():  # the requested slot has been answered
